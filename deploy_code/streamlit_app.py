# app_slots_priced_two_keys.py
# - Places (New) autocomplete + details  [BACKEND_KEY]
# - KMeans clustering (~target/cluster)
# - TSP per cluster (OR-Tools) or VRPTW per cluster (time windows)
# - Google route polylines (chunked ≤25 waypoints)  [BACKEND_KEY]
# - NEW ORDER: Offer PRICED TIME SLOTS (one per run) → book a slot → update ONLY that run
# - Robust map rendering with forced remounts  [MAPS_JS_KEY]
# - Two-key model (safer): GOOGLE_MAPS_JS_KEY (frontend), GOOGLE_BACKEND_KEY (server)
...
# - GOOGLE_MAPS_JS_KEY (frontend key; restrict by HTTP referrer; API = Maps JavaScript)
# - GOOGLE_BACKEND_KEY (server key; restrict by API (Places, Routes); ideally by static egress IP)
...
# ---------- Google helpers (use BACKEND_KEY) ----------
...
# ---------- Route metrics (Euclidean-based sim for Δtime/Δkm) ----------
...
# ---------- Polyline (chunked; use BACKEND_KEY) ----------
...
# ---------- Robust map renderer (uses MAPS_JS_KEY) ----------
...
if not BACKEND_KEY or not MAPS_JS_KEY:
    st.info("Add your Google keys: **GOOGLE_BACKEND_KEY** (Places & Routes, server-only) and **GOOGLE_MAPS_JS_KEY** (Maps JavaScript, browser).")
...
if not st.session_state.chosen:
    ...
    for c in st.session_state.chosen:
        pid=c.get("place_id")
        cfg=st.session_state.tw.setdefault(pid, {"priority": False, "start_min": 9*60, "end_min": 12*60, "service_min": 5})
        with st.expander(f"Constraints — {c['address']}", expanded=False):
            ...
            with colD:
                cfg["service_min"]=st.number_input("Service (min)",0,120,int(cfg.get("service_min",5)), key=f"svc_{pid}")
...
if clusters and st.button("Solve routes per cluster"):
    ...
    for cl_id in range(k):
        ...
        # Assemble service & time windows
        svc,tws,twe=[],[],[]
        for p in cl_pts:
            cfg = st.session_state.tw.get(p.get("place_id",""))
            if cfg:
                svc.append(int(cfg.get("service_min", 5))*60)
                if enforce_windows:
                ...
                svc.append(5*60); tws.append(0); twe.append(horizon_sec)
...
if not st.session_state.routes:
    ...
    if selected2:
        ...
        if det2.get("error"):
            ...
            pid = det2["place_id"]
            st.session_state.tw.setdefault(pid, {"priority": False,"start_min": 9*60,"end_min": 12*60,"service_min": 5})
    ...
    def build_slots_for_candidate(cand_point: Dict[str,Any]) -> List[Dict[str,Any]]:
        ...
        for rr in routes:
            ...
            svc_c = int(cfg_c.get("service_min", 5))*60
            ...
            for p in base_points:
                cfg = st.session_state.tw.get(p.get("place_id","")) or {}
                svc_base.append(int(cfg.get("service_min", 5))*60)
                if enforce_windows:
            ...
            svc_new = [ (st.session_state.tw.get(p.get("place_id","")) or {}).get("service_min", 5)*60 for p in new_points_order ]
            if enforce_windows:
            ...
            # One slot per run (arrival time = model’s predicted arrival)
            slot = {
                "cluster": cl_id,
                ...
                "end_sec": int(cand_arrival + max(60, svc_c)),  # slot = service time
